...
このアプリケーションの品質を保証するために、以下のテスト戦略を推奨します。

### ユニットテスト
個々の関数やコンポーネントを分離してテストします。
- **対象**: `src/lib/utils.ts` 内のヘルパー関数 (`formatPriceLevel`など)、AIフロー内のデータ整形ロジックなど。
- **ツール**: `Vitest` や `Jest` などのテストフレームワークを使用します。

### 結合テスト
複数のコンポーネントやモジュールが連携して正しく動作するかをテストします。
- **対象**: `src/app/actions.ts` のサーバーアクション。外部API (`Google Places`, `Genkit`)をモック化し、全体のオーケストレーションロジックを検証します。
- **ツール**: `React Testing Library` と `Vitest`/`Jest` を組み合わせて使用します。

### E2E (End-to-End) テスト
実際のユーザー操作をシミュレートし、アプリケーション全体の流れをテストします。
- **シナリオ例**:
  1. トップページにアクセスする。
  2. 検索フォームに条件を入力し、検索ボタンをクリックする。
  3. おすすめのお店が表示されることを確認する。
  4. 詳細を開き、Google Mapsへのリンクが正しいことを確認する。
- **ツール**: `Cypress` や `Playwright` を使用します。

### AIフローのテスト
Genkitには開発用のUIが付属しており、これを使って個別のフローを対話的にテストできます。
```bash
npm run genkit:dev
```
ブラウザで `http://localhost:4000` を開くと、各フローの入力と出力を確認できます。
